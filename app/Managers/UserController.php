...
class UserController extends Manager
{
    ...
    public function lengyamList($request, $response)
    {
        ...
        // CRITICAL FIX: Force streaming output to bypass web server buffer limits
        while (ob_get_level()) {
            ob_end_clean();
        }
        ...
        $jsonResponse = json_encode(array("leng_yam" => $WanSpecial, "next_wanpra" => $nextWanpra, "wan_pras" => $objWanprasx), JSON_UNESCAPED_UNICODE);
        ...
        // Return empty response since we already echoed
        return $response;
    }
