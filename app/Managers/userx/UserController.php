...
class UserController extends Manager
{
    ...
    public function dressColor($request, $response)
    {
        ...
        if (!empty($numbDays)) {
            // Split days string into array of individual IDs
            $ids = str_split($numbDays);
            $uniqueIds = array_unique($ids);

            if (!empty($uniqueIds)) {
                // Optimization: Batch fetch all colors in a single query to avoid N+1 bottleneck
                ...
                if ($stmt) {
                    ...
                    // Map results by colorid for efficient lookup
                    $colorMap = [];
                    foreach ($rows as $row) {
                        $colorMap[$row['colorid']] = $row;
                    }

                    // Reconstruct the ordered list, including duplicates as per original behavior
                    foreach ($ids as $id) {
